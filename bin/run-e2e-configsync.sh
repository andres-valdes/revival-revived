#!/bin/sh
# Config-sync multiplayer E2E: the host sets a distinctive revive config; the
# client joins with a different local config and must adopt the host's
# (server-authoritative). Two processes over the CustomSocket backend.
set -u
...
echo "=== launching HOST ==="
env RR_E2E=1 RR_E2E_ROLE=host RR_E2E_SCENARIO=configsync RR_E2E_PORT="$PORT" RR_E2E_WORLD=e2e_mp \
    RR_E2E_RESULTS="$HOST_RES" RR_E2E_LOG="$HOST_E2E" \
    timeout 420 ./run_bepinex.sh ./valheim.x86_64 $COMMON_ARGS > "$HOST_OUT" 2>&1 &
HOST_PID=$!
...
env RR_E2E=1 RR_E2E_ROLE=client RR_E2E_SCENARIO=configsync RR_E2E_PORT="$PORT" RR_E2E_HOST=127.0.0.1 RR_E2E_WORLD=e2e_mp \
    RR_E2E_RESULTS="$CLI_RES" RR_E2E_LOG="$CLI_E2E" \
    timeout 400 ./run_bepinex.sh ./valheim.x86_64 $COMMON_ARGS > "$CLI_OUT" 2>&1 &
CLI_PID=$!
