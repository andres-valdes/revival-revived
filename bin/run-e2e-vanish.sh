...
# Long window: the marker must be destroyed by DISCONNECT semantics only,
# never by window expiry racing the test choreography.
export RR_E2E_WINDOW=90
COMMON_ARGS="-screen-fullscreen 0 -screen-width 640 -screen-height 480"

echo "=== launching HOST ==="
env RR_E2E=1 RR_E2E_ROLE=host RR_E2E_SCENARIO=vanish RR_E2E_PORT="$PORT" RR_E2E_WORLD="e2e_vanish_$(date +%s)" \
...
env RR_E2E=1 RR_E2E_ROLE=client RR_E2E_SCENARIO=vanish RR_E2E_PORT="$PORT" RR_E2E_HOST=127.0.0.1 RR_E2E_WORLD="e2e_vanish_$(date +%s)" \
