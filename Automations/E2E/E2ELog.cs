...
/// file given by AUTO_E2E_LOG. The file path is unique per process, so two
...
public static class E2ELog {
    ...
    private static string? Path {
        get {
            if (!s_init) {
                s_path = Environment.GetEnvironmentVariable("AUTO_E2E_LOG");
                if (string.IsNullOrEmpty(s_path)) {
                    var res = Environment.GetEnvironmentVariable("AUTO_E2E_RESULTS");
